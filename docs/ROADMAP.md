...
| 2026-08-07 | Homebrew distribution, `v0.2.0` | `brew install xgentic/tap/apm-registry` from [xgentic/homebrew-tap](https://github.com/xgentic/homebrew-tap). The formula is generated by `make brew-formula` from the **published** `SHA256SUMS` — never a local rebuild, which stamps a different build date and so a different digest |
| | ↳ TR-32 | `APM_REGISTRY_ADDR` and `serve -addr`, so `brew services` can pin `127.0.0.1`. Default unchanged (every interface on `PORT`). `-port` replaces only the port half, so it cannot widen a loopback binding; `PORT` disagreeing with `APM_REGISTRY_ADDR` fails startup rather than picking a winner |
| | | |
